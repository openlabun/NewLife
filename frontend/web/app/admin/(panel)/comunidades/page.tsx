...
import {
  ...
  changeMemberModerator, Community, Member
...
export default function ComunidadesPage() {
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      const detail = await getCommunity(viewingCommunity.id)
      setCommunityDetail(detail)
  ...
  if (viewingCommunity) {
    ...
    return (
      <div className="space-y-6">
        <Button variant="ghost" onClick={() => { setViewingCommunity(null); setCommunityDetail(null) }}
        ...
        <Card className="bg-white border-[#e5e5e5] shadow-sm">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Nombre", value: viewingCommunity.nombre },
                { label: "Estado", value: viewingCommunity.activa ? "activa" : "inactiva", isBadge: true },
                { label: "Creada por", value: viewingCommunity.creado_por },
                { label: "Fecha de creación", value: viewingCommunity.created_at
                    ? new Date(viewingCommunity.created_at).toLocaleDateString('es-CO')
                    : '—' },
              ].map(({ label, value, isBadge }) => (
                <div key={label} className="p-4 bg-[#f8f6f3] rounded-lg">
                  <p className="text-sm text-[#a3a3a3]">{label}</p>
                  {isBadge ? (
                    <Badge className={value === 'activa'
                  ...
                  {members.length === 0 ? (
                    ...
                    members.map((member) => (
                      <TableRow key={member.id} className="border-[#e5e5e5] hover:bg-[#f8f6f3]">
                        <TableCell className="text-[#a3a3a3] font-mono text-xs">{member.usuario_id}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Badge className={member.es_moderador
                        ...
                        <TableCell className="text-[#737373] text-sm">
                          {member.joined_at ? new Date(member.joined_at).toLocaleDateString('es-CO') : '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button size="sm" variant="outline" onClick={() => handleRemoveMember(member)}
        ...
        <Dialog open={showAddMemberModal} onOpenChange={setShowAddMemberModal}>
          <DialogContent className="bg-white border-[#e5e5e5]">
            <DialogHeader>
              <DialogTitle className="text-[#1a1a1a]">Agregar miembro</DialogTitle>
              <DialogDescription className="text-[#737373]">Ingresa el correo del usuario</DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <div className="space-y-2">
                <Label className="text-[#1a1a1a]">Correo electrónico</Label>
                <Input type="email" placeholder="usuario@email.com" value={memberEmail}
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id="moderator" checked={memberIsMod}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowAddMemberModal(false)}
                className="border-[#e5e5e5] text-[#737373]">Cancelar</Button>
        ...
        <Dialog open={showAccessModal} onOpenChange={setShowAccessModal}>
          <DialogContent className="bg-white border-[#e5e5e5]">
            <DialogHeader>
              ...
              <DialogDescription className="text-[#737373]">{selectedMember?.usuario_id}</DialogDescription>
            </DialogHeader>
            <div className="py-4">
        ...
        <Dialog open={showModModal} onOpenChange={setShowModModal}>
          <DialogContent className="bg-white border-[#e5e5e5]">
            ...
            <div className="py-4 flex items-center space-x-3">
              <Switch id="mod-switch" checked={selectedMod} onCheckedChange={setSelectedMod}
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-white border-[#e5e5e5] shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={() => setShowNewCommunityModal(true)}
            className="bg-[#d4854a] hover:bg-[#c07842] text-white gap-2">
              ...
              {communities.length === 0 ? (
                ...
                communities.map((community) => (
                  <TableRow key={community.id} className="border-[#e5e5e5] hover:bg-[#f8f6f3]">
                    ...
                    <TableCell className="text-[#737373] max-w-xs truncate">{community.descripcion || '—'}</TableCell>
                    <TableCell className="text-[#737373]">{community.total_miembros ?? 0}</TableCell>
                    <TableCell>
                      <Badge className={community.activa
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="outline" onClick={() => openDetail(community)}
      ...
      <Dialog open={showNewCommunityModal} onOpenChange={setShowNewCommunityModal}>
        <DialogContent className="bg-white border-[#e5e5e5]">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#737373]">Ingresa los datos de la nueva comunidad</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              <Label className="text-[#1a1a1a]">Nombre</Label>
              <Input placeholder="Nombre de la comunidad" value={newName}
                onChange={(e) => setNewName(e.target.value)}
                className="bg-[#f8f6f3] border-[#e5e5e5] text-[#1a1a1a]" />
            </div>
            <div className="space-y-2">
              <Label className="text-[#1a1a1a]">Descripción</Label>
              <Textarea placeholder="Descripción de la comunidad" value={newDescription}
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="bg-white border-[#e5e5e5]">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#737373]">Modifica los datos de la comunidad</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              <Label className="text-[#1a1a1a]">Nombre</Label>
        ...
        <DialogContent className="bg-white border-[#e5e5e5]">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#737373]">
              ¿Estás seguro de eliminar <strong className="text-[#1a1a1a]">{selectedCommunity?.nombre}</strong>?
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
