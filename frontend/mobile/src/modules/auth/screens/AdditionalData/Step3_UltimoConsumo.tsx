...
export default function Step3_UltimoConsumo({ navigation }: any) {
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      cells.push(
        <TouchableOpacity
          key={day}
          style={[styles.dayCell, selected && styles.dayCellSelected, isFuture && styles.dayCellDisabled]}
          onPress={() => { if (!isFuture) { setSelectedDate(date); setError(''); } }}
          disabled={isFuture}
        >
          <Text style={[styles.dayText, selected && styles.dayTextSelected, isFuture && styles.dayTextDisabled]}>
            {day}
          </Text>
        </TouchableOpacity>
      );
  ...
  const handleContinue = () => {
    setError('');
    if (!selectedDate) {
      setError('Por favor selecciona la fecha de tu último consumo para calcular tu tiempo de sobriedad.');
      return;
    }
  ...
  return (
    <StepLayout
      ...
      characterImage={hasDate ? require('../../../../assets/images/character4.png') : require('../../../../assets/images/character3.png')}
...
const styles = StyleSheet.create({
  ...
  datePlaceholder: { color: colors.border },
  clearIcon: { fontSize: 14, color: colors.textMuted },
  calendar: { backgroundColor: colors.inputBackground, borderRadius: borderRadius.md, padding: spacing.md },
  calendarHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.md },
  navArrow: { fontSize: fontSizes.lg, color: colors.text, paddingHorizontal: spacing.sm },
  monthTitle: { fontSize: fontSizes.md, fontWeight: '700', color: colors.text },
  weekDays: { flexDirection: 'row', marginBottom: spacing.xs },
  weekDay: { flex: 1, textAlign: 'center', fontSize: fontSizes.sm, fontWeight: '600', color: colors.textMuted },
  daysGrid: { flexDirection: 'row', flexWrap: 'wrap' },
  dayCell: { width: `${100 / 7}%`, aspectRatio: 1, justifyContent: 'center', alignItems: 'center', borderRadius: borderRadius.sm },
  dayCellSelected: { backgroundColor: '#4A90D9' },
  dayText: { fontSize: fontSizes.sm, color: colors.text },
