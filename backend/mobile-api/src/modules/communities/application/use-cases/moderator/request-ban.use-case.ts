...
export class ModRequestBanUseCase {
  ...
  async execute(comunidadId: string, targetId: string, moderadorUuid: string, motivo: string) {
    ...
    if (membRows.length === 0) throw new NotFoundException('El usuario no es miembro de esta comunidad.');
    ...
    if (pendiente) throw new BadRequestException('Ya existe una solicitud de baneo pendiente para este usuario.');
 
    await this.dbService.insert('solicitudes_baneo', [{
      usuario_id:   targetId,
      moderador_id: moderadorRobleId,
